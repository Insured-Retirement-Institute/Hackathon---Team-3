...
import { DataAlert, IAlertBreakdown, INotification } from '../../../../../../models/notifications';
import { SystemMessage } from '../../../../../../models';

@Component({
    selector: 'app-data-alerts-total',
    imports: [ChartModule],
    templateUrl: './data-alerts-total.component.html',
    styleUrl: './data-alerts-total.component.scss'
})
export class DataAlertsTotalComponent {
    ...
    dataSelect(e: any): void {
        const alert: string = DataAlert[this.chartData.labels[e.element.index]];
        const alertBreakdown: IAlertBreakdown = {
            id: Number(alert),
            alert: DataAlert[Number(alert)],
            alertDescription: '',
            alertCount: 1
        };
